...
if exists('D:/Projects/GAGESii_ANNstuff/Data_Out/Daymet_Annual.csv'):
    Daymet_Annual = pd.read_csv(
...
df_test_in_sites = pd.read_csv(
    f'{dir_WY}/yrs_98_12/annual_WY/Ann_WY_test_in.csv',
    dtype={"site_no":"string"}
    )['site_no']

# test basins not used in training
df_test_nit_sites = pd.read_csv(
    f'{dir_WY}/yrs_98_12/annual_WY/Ann_WY_test_nit.csv',
    dtype={"site_no":"string"}
    )['site_no']
...
if not exists('D:/Projects/GAGESii_ANNstuff/Data_Out/Daymet_Annual.csv'):
    pd.DataFrame({
        'year': [],
...
for shape in list_shp:
    ...
    for gg_id in shp_in['GAGE_ID']:
        ...
        if Daymet_Annual['site_no'].str.contains(gg_id).any():
            continue
        ...
        if df_all_sites.str.contains(gg_id).any():
            ...
            pd.DataFrame({
                'year': np.array(range(1998, 2013, 1)), # this line may need to be edited a bit
                'site_no': np.repeat(gg_id, len(annual.time)).tolist(),
                'tmin_retreived': annual.tmin.mean(['x', 'y']).values.tolist(),
                'tmax_retreived': annual.tmax.mean(['x', 'y']).values.tolist(),
                'prcp_retreived': annual.prcp.mean(['x', 'y']).values.tolist(),
                'vp_retreived': annual.vp.mean(['x', 'y']).values.tolist(),
                'swe_retreived': annual.swe.mean(['x', 'y']).values.tolist()
                # 'srad_retreived': [annual.srad.mean(['x', 'y']).values.tolist()],
                # 'dayl_retreived': [annual.dayl.mean(['x', 'y']).values.tolist())]
            }).to_csv(
                'D:/Projects/GAGESii_ANNstuff/Data_Out/Daymet_Annual.csv',
                mode = 'a',
                index = False,
...
# all_results.to_csv("D:/Projects/GAGESii_ANNstuff/Data_Out/Daymet_Annual.csv")
