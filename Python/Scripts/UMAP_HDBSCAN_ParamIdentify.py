...
The script, UMAP_HDBSCAN_GetLabels.py is then used to read in the
...
some info on HDBSCAN parameters:
...
df_expl = df_expl.drop(
    df_expl.columns[
        df_expl.columns.str.contains('tmin') |
        df_expl.columns.str.contains('tmax') |
        df_expl.columns.str.contains('prcp') |
        df_expl.columns.str.contains('vp') |
        df_expl.columns.str.contains('swe')
        ],
    axis = 1
    )
...
param_grid = {
    ...
    'umap_n_components': [3, 10, 20, 30],
    'hd_min_cluster_size': [30, 45, 60],
    'hd_min_samples': [1],
...
for i in combosAllVars.itertuples(index = False):
    ...
    # append absolute validity index to list to later add to dataframe
...
df_plot = pd.DataFrame(umap_reducer.embedding_)
df_plot['clusters'] = hd_clusterer.labels_
...
paramNat_grid = {
    ...
    'umap_metric': ['euclidean', 'cosine'],
    'umap_n_components': [3, 10, 20, 30],
    'hd_min_cluster_size': [30, 45, 60], # [120, 180, 240], # Set larger here because will later susbet clusters
    'hd_min_samples': [1],
    'hd_metric': ['euclidean'], #, 'manhattan'],
    'hd_cluster_selection_epsilon': [0], #, 0.2, 0.4, 0.6],
    'hd_cluster_selection_method': ['leaf', 'eom']  # 'leaf' (smaller homogeneous) or 'eom' (fewer clusters)
...
# subset to best performing parameter sets
# drop duplicated results
combosout_df.drop_duplicates(
    subset = ['Validity_Ind', 'Rel_Validity_Ind', 'Noise_Count', 'Numb_Clusters'],
    inplace = True)
    
combosout_df.reset_index(drop = True, inplace = True)

combosout_df.sort_values(by = 'Validity_Ind', ascending = False)
...
# define empty dataframe to append parameters with which to work in the next step
combosNatBest = pd.DataFrame(columns = combosout_df.columns)


# filter to only highest validity index for each unique number of clusters
for i in combosout_df['Numb_Clusters'].unique():
    print(i)
    # subset to clusters = i
    tmp = combosout_df[combosout_df['Numb_Clusters'] == i]
    # keep only max
    tmp = tmp[tmp['Validity_Ind'] == np.max(tmp['Validity_Ind'])]

    combosNatBest = pd.concat([combosNatBest, tmp])
...
# write results sorted by validity index to csv
##############
combosNatBest.to_csv(
        'D:/Projects/GAGESii_ANNstuff/Data_Out/'
        'UMAP_HDBSCAN/UMAP_HDBSCAN_NatVars_ParamSearch_Results.csv',
        index = False
    )


# if above already executed, then read in combosAllVars 
combosNatBest = pd.read_csv(
    'D:/Projects/GAGESii_ANNstuff/Data_Out/'
    'UMAP_HDBSCAN/UMAP_HDBSCAN_NatVars_ParamSearch_Results.csv',
    )
...
# %%
# UMAP -> HDBSCAN on Anthropogenic vars
###############

# parameters for the anthropogenic feature pipeline search
paramAnth_grid = {
    'umap_n_neighbors': [30, 60, 90], # for initial run do 6 for each
    'umap_min_dist': [0],
    'umap_metric': ['euclidean', 'cosine'],
...
#####
# loop through parameter grid for anthropogenic features identifying best param sets
######

# produce all combos of parameters
combos = product(*(paramAnth_grid[Name] for Name in paramAnth_grid))
combosAnth = pd.DataFrame(combos)
...
df_work = df_expl[df_expl.columns.intersection(anthro_feats)]



combosout_df = combosAnth
for i in combosout_df.itertuples(index = False):
    tempParams = np.array(i)
    # define standard scaler
    scaler = StandardScaler()
...
combosout_df['Noise_Count'] = noise_count
combosout_df['Numb_Clusters'] = num_clusters
combosout_df['VarType'] = np.repeat('Anthro', combosout_df.shape[0])
...
# define empty dataframe to append parameters with which to work in the next step
combosAnthBest = pd.DataFrame(columns = combosAnth.columns)
...
for i in combosout_df['Numb_Clusters'].unique():
    ...
    combosAnthBest = pd.concat([combosAnthBest, tmp])

print(combosAnthBest)
...
# write results sorted by validity index to csv
##############
combosAnthBest.to_csv(
        'D:/Projects/GAGESii_ANNstuff/Data_Out/'
        'UMAP_HDBSCAN/UMAP_HDBSCAN_AnthroVars_ParamSearch_Results.csv',
        index = False
    )


# if above already executed, then read in combosAllVars 
combosAnthBest = pd.read_csv(
    'D:/Projects/GAGESii_ANNstuff/Data_Out/'
    'UMAP_HDBSCAN/UMAP_HDBSCAN_AnthroVars_ParamSearch_Results.csv',
    )
...
# # THIS CODE WAS ORIGINALLY INTENDED TO BE EXECUTED IMMEDIATELY FOLLOWING 
# # CLUSTERING BASED ON NATURAL VARS. IT WAS GOING TO FURTHER CLUSTER EACH
# # OF THE NATURAL-BASED CLUSTERS BASED ON ANTHROPOGENIC VARS
...
# # define counter to track progress
# cntNat = 0
...
#     # print progress
#     print(f'starting Nat param set {cntNat}/{combosNatBest.shape[0]}')
#     cntNat += 1
