...
time_scale = ['mean_annual'] # , 'annual', 'monthly'] # 'mean_annual', 'annual', 'monthly', 'daily'
...
for c in clust_meth:
    for ts in time_scale:
        ...
        ind_out_name = f'{dir_out}/All_IndResults_{ts}.pkl'
        ...
        # define output file path and name for independent catchment results
        sum_out_name = f'{dir_out}/All_SummaryResults_{ts}.pkl'
        ...
        ind_test = pd.read_pickle(
            f'{dir_out}/All_IndResults_{ts}.pkl'
        )

        summ_test = pd.read_pickle(
            f'{dir_out}/All_SummaryResults_{ts}.pkl'
        )
